...
void ImageDev::load( void* input )
{
    cudaMemcpy(_input_image_d.data, input, _w*_h, cudaMemcpyDeviceToDevice); // assume that host Plane2D has no pitch
}

void ImageDev::resetDimensions( int w, int h )
{
    if( _max_w == 0 && _max_h == 0 ) {
        _max_w = _w = w;
        _max_h = _h = h;
...
void ImageDev::allocate( int w, int h )
{
    nvtxRangePushA( "allocating host-side image memory" );
...
ImageFloatDev::ImageFloatDev( )
  : ImageBase( 0, 0 )
{
}

ImageFloatDev::ImageFloatDev( int w, int h )
  : ImageBase( w, h )
{
    allocate( w, h );
}
...
void ImageFloatDev::load( void* input )
{
    /* The host memcpy may seem like a really stupid idea, but _input_image_h
     * is in CUDA-allocated pinned host memory, which makes the H2D copy
     * much faster.
...
//    memcpy( _input_image_h.data, input, _w*_h*sizeof(float) ); // assume that host Plane2D has no pitch
//    _input_image_h.memcpyToDevice( _input_image_d );
    cudaMemcpy(_input_image_d.data, input, _w*_h*sizeof(float), cudaMemcpyDeviceToDevice);
}

void ImageFloatDev::resetDimensions( int w, int h )
{
    if( _max_w == 0 && _max_h == 0 ) {
        _max_w = _w = w;
        _max_h = _h = h;
...
void ImageFloatDev::allocate( int w, int h )
{
    nvtxRangePushA( "allocating host-side image memory" );
...
void ImageFloatDev::destroyTexture( )
{
    cudaError_t err;
    err = cudaDestroyTextureObject( _input_image_tex );
    POP_CUDA_FATAL_TEST( err, "Could not destroy texture object: " );
...
void ImageFloatDev::createTexture( )
{
    /* initializing texture for upscaling
     */
    memset( &_input_image_texDesc, 0, sizeof(cudaTextureDesc) );
